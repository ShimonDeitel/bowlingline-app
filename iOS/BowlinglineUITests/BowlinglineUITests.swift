...
final class BowlinglineUITests: XCTestCase {
    ...
    func testFreeLimitTriggersPaywall() throws {
        ...
        for i in 0..<12 {
            ...
            if titleField.waitForExistence(timeout: 3) {
                ...
                app.buttons["saveEntryButton"].tap()
            } else if app.staticTexts["Bowlingline Pro"].waitForExistence(timeout: 3) {
                break
            }
        }
        XCTAssertTrue(app.staticTexts["Bowlingline Pro"].waitForExistence(timeout: 5))
    }
