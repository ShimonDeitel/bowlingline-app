import XCTest
@testable import Bowlingline

@MainActor
final class BowlinglineTests: XCTestCase {
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(OutingEntry(title: "Test", vendor: "V", amount: 10, date: Date()))
    ...
    func testCannotAddMoreAtLimitWithoutPro() {
        ...
        while store.entries.count < Store.freeLimit {
            store.add(OutingEntry(title: "Filler", vendor: "V", amount: 1, date: Date()))
        }
    ...
    func testDeleteEntryRemovesIt() {
        let store = Store()
        let entry = OutingEntry(title: "ToDelete", vendor: "V", amount: 5, date: Date())
        store.add(entry)
        store.delete(entry)
    ...
    func testUpdateEntryChangesFields() {
        let store = Store()
        var entry = OutingEntry(title: "Original", vendor: "V", amount: 5, date: Date())
        store.add(entry)
        entry.title = "Updated"
        store.update(entry)
